# CloudNotes

> 一个使用*docsifyjs*创建的基于markdown的笔记网站

[![GitHub package.json version](https://img.shields.io/github/package-json/v/yequanrui/CloudNotes)](https://github.com/yequanrui/CloudNotes '跳转主页')
[![GitHub watchers](https://img.shields.io/github/watchers/yequanrui/CloudNotes?style=social)](https://github.com/yequanrui/CloudNotes/watchers)
[![GitHub stars](https://img.shields.io/github/stars/yequanrui/CloudNotes?style=social)](https://github.com/yequanrui/CloudNotes/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/yequanrui/CloudNotes?style=social)](https://github.com/yequanrui/CloudNotes/network/members)

[![docsify](https://img.shields.io/npm/v/docsify?label=docsify)](https://docsify.js.org/ '跳转主页')
[![docsify-cli](https://img.shields.io/npm/v/docsify-cli?label=docsify-cli)](https://cli.docsifyjs.org/ '跳转主页')

# 插件

[![docsify-copy-code](https://img.shields.io/npm/v/docsify-copy-code?label=docsify-copy-code)](https://github.com/jperasmus/docsify-copy-code '跳转主页')
[![docsify-count](https://img.shields.io/npm/v/docsify-count?label=docsify-count)](https://github.com/827652549/docsify-count '跳转主页')
[![docsify-edit-on-github](https://img.shields.io/npm/v/docsify-edit-on-github?label=docsify-edit-on-github)](https://github.com/njleonzhang/docsify-edit-on-github '跳转主页')
[![docsify-footer](https://img.shields.io/npm/v/@alertbox/docsify-footer?label=docsify-footer)](https://github.com/alertbox/docsify-footer '跳转主页')
[![docsify-pagination](https://img.shields.io/npm/v/docsify-pagination?label=docsify-pagination)](https://github.com/imyelo/docsify-pagination '跳转主页')
[![docsify-scroll-to-top](https://img.shields.io/npm/v/docsify-scroll-to-top?label=docsify-scroll-to-top)](https://github.com/zhengxiangqi/docsify-scroll-to-top '跳转主页')
[![docsify-share](https://img.shields.io/npm/v/docsify-share?label=docsify-share)](https://github.com/coroo/docsify-share '跳转主页')
[![docsify-tabs](https://img.shields.io/npm/v/docsify-tabs?label=docsify-tabs)](https://github.com/jhildenbiddle/docsify-tabs '跳转主页')
[![docsify-updated](https://img.shields.io/npm/v/docsify-updated?label=docsify-updated)](https://github.com/pfeak/docsify-updated '跳转主页')
[![prismjs](https://img.shields.io/npm/v/prismjs?label=prismjs)](https://prismjs.com/ '跳转主页')

## 主题

[![docsify-darklight-theme](https://img.shields.io/npm/v/docsify-darklight-theme?label=docsify-darklight-theme)](https://github.com/boopathikumar018/docsify-darklight-theme '跳转主页')
[![prism-themes](https://img.shields.io/npm/v/prism-themes?label=prism-themes)](https://github.com/PrismJS/prism-themes '跳转主页')
